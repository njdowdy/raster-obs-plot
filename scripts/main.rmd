...
grd_files <- list.files("./input/extracted/range_data_rasters/", pattern = "\\.grd$") # generate a list of raster locations
...
new_df <- new_df %>% mutate(num_records_to_sample = (8000000 * record_prop) - n) # calculate number of undigitized records
...
for (taxon in taxa) { # for each taxon in the csv file:
  ...
  new_points <- coordinates[sample(nrow(coordinates), num_records_to_sample, replace = TRUE), ] # sample the coordinate blocks to create new lat/long points
  ...
  names(new_points) <- c("long", "lat") # change the column names for easy referencing
  ...
  # store values for writing
  name <- c(name, taxon)
  new_long_list <- c(new_long_list, new_points$long)
  new_lat_list <- c(new_lat_list, new_points$lat)
}

  # add an occurrence_id column (1:length(df))
  ## WRITE OUT INTO CSV
  # occurrence_id, name, lat, long
